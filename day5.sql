...
SELECT e.employee_first_name AS firstname,a.state AS "employee state" FROM employees e,addresses a 
...
SELECT company, COUNT(company) FROM workers
WHERE salary>2000
GROUP BY company
--Find the minimum and maximum salary for every company
SELECT COMPANY,MIN(salary) AS minumum_salary,MAX(salary) AS maximum_salary FROM workers
GROUP BY company
--HAVING CLAUSE
...
SELECT company,COUNT(company) FROM workers
GROUP BY company
HAVING COUNT(company)>1-->HAVING is for filtering(if) for group by. After HAVING we have to use aggragate function. DO not use field names.
--Find the minimum salary if it is more than 2000 for every company
SELECT company, MIN(salary) AS minimum_salary FROM workers
GROUP BY company
HAVING MIN(salary)>2000
--Find the maximum salary if it is less than 3000 for every state
SELECT state,MAX(salary) AS maximum_salary_state FROM workers
GROUP BY state
...
SELECT state AS "state/name" FROM workers
WHERE salary>3000
